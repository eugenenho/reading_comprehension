...
def train_model():
    ...
    for iter_num in range(1, simple_configs.FULL_PASS_ITERS + 1):
        ...
        for i in tqdm(range(1, simple_configs.MAX_NUM_ITERS + 1)):
            X_batch = X_train[i:(i + 1)*simple_configs.TRAIN_BATCH_SIZE]
            Y_batch = Y_train[i:(i + 1)*simple_configs.TRAIN_BATCH_SIZE, :]
...
def build_data():
    ...
    Y_train = np.zeros((simple_configs.NUM_QUESTIONS, simple_configs.OUTPUT_MAX_LENGTH, simple_configs.MAX_NB_WORDS))
    for r, row in enumerate(Y_train_indexes):
        for c, i in enumerate(row):
            Y_train[r][c][i] = 1

    print 'Made Data Matrices'
